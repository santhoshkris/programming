...
def update_files():
    ...
    with fileinput.FileInput(MAINT_JS_FILE, inplace=True) as f:
        for line in f:
    ...
    result = (subprocess.run(['node', 'maintenance-update.js'], capture_output=True, encoding='UTF-8').stdout).split('\n')
    ...
    for idFile in ID_LANG_FILES:
        ...
        with fileinput.FileInput(idFile, inplace=True) as f:
            for line in f:
    ...
    for enFile in EN_LANG_FILES:
        ...
        with fileinput.FileInput(enFile, inplace=True) as f:
            for line in f:
    ...
    for pFile in PROP_FILES:
        ...
        if "stage." in pFile:
            ...
            staging = False
        with fileinput.FileInput(pFile, inplace=True) as f:
            for line in f:
...
def git_complete():
    ...
    repo.git.add('--all')
    repo.git.commit('-m', 'maint banner from py script')
